...
// It will use the VITE_API_URL when deployed, or fallback to localhost for local development.
...
function CommitteePage() {
  ...
  // --- Effect 1: For fetching the initial committee data ---
  useEffect(() => {
    setIsLoading(true);
    // INSTANCE 1: Used in the initial data fetch
  ...
  // --- Effect 2: For managing the Socket.IO connection ---
  useEffect(() => {
    if (!committee) return;

    // INSTANCE 2: Used to connect the socket
    const socket = io(apiUrl);
    socket.emit('join_committee', committee.id);
    socket.on('bid_updated', (updatedCommittee) => {
  ...
  const handlePlaceBid = () => {
    ...
    // INSTANCE 3: A temporary socket for emitting the event
    // In a larger app, you'd share the socket instance, but this works perfectly for this case.
    const socket = io(apiUrl);
    socket.emit('new_bid', {
